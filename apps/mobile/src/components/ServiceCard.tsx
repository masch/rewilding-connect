...
export function ServiceCard({ service, isEditing, onPress, accessibilityLabel }: ServiceCardProps) {
  ...
  return (
    <Button
      variant="ghost"
      className="bg-surface-container-low mb-6 rounded-3xl overflow-hidden border border-outline-variant/20 shadow-sm p-0 min-w-0"
      ...
      <View className="w-full">
        ...
        <View className="p-5">
          ...
          <View className="flex-row items-center justify-between">
            ...
            {/* Visual-only Button (interaction handled by parent card Button) */}
            <View
              className={`
              px-6 rounded-2xl h-11 flex-row items-center justify-center gap-2
              ${isEditing ? "bg-secondary/10 border border-secondary" : "bg-primary-container"}
            `}
            >
              <MaterialCommunityIcons
                name={isEditing ? "pencil" : "plus"}
                size={18}
                color={isEditing ? COLORS.secondary : "white"}
              />
              <Text
                className={`font-display font-bold ${isEditing ? "text-secondary" : "text-on-primary"}`}
              >
